...
template <int ORDER>
void BTree<ORDER>::inorder(BTreeNode* node, vector<pair<string, string>>& pairs) const
{
    if (!node) return;
    ...
    {
        if (!node->isLeaf())
            inorder(node->children[i], pairs);

        pairs.emplace_back(node->keys[i], node->values[i]);
    }
    
    //Desni cvor
    if (!node->isLeaf())
        inorder(node->children[node->numKeys], pairs);
}