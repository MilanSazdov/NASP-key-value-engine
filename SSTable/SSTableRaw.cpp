...
SSTableRaw::SSTableRaw(const std::string& dataFile,
	...
	Block_manager* bmp,
	bool is_single_file)
	: SSTable(dataFile, indexFile, filterFile, summaryFile, metaFile, bmp, is_single_file, false)
{
}
...
std::vector<IndexEntry> SSTableRaw::writeDataToBuffer(std::vector<Record>& sortedRecords, std::ostream& out) {
    std::vector<IndexEntry> index_entries;
    uint64_t logical_offset = 0;
    ...
    for (const auto& record : sortedRecords) {
        ...
        // Serijalizacija jednog zapisa
        Wal_record_type flag = Wal_record_type::FULL; // U bafer pisemo kao da nema deljenja
        size_t header_len = sizeof(record.crc) + sizeof(flag) + sizeof(record.timestamp) +
            sizeof(record.tombstone) + sizeof(record.key_size) + sizeof(record.value_size);
        size_t total_record_len = header_len + record.key_size + record.value_size;

        out.write(reinterpret_cast<const char*>(&record.crc), sizeof(record.crc));
        ...
        out.write(reinterpret_cast<const char*>(&record.key_size), sizeof(record.key_size));
        out.write(reinterpret_cast<const char*>(&record.value_size), sizeof(record.value_size));
        out.write(record.key.data(), record.key_size);
        out.write(record.value.data(), record.value_size);
...
std::vector<IndexEntry> SSTableRaw::writeIndexToBuffer(std::ostream& out) {
    std::vector<IndexEntry> summary_entries;
    if (index_.empty()) return summary_entries;

    summary_entries.reserve(index_.size());

    // Format: [broj_unosa(uint64)][key_size_1(uint64), key_1, offset_1(uint64)]...
    uint64_t count = index_.size();
    out.write(reinterpret_cast<const char*>(&count), sizeof(count));
    uint64_t current_offset_in_index = sizeof(count);

    for (const auto& entry : index_) {
        summary_entries.push_back({ entry.key, current_offset_in_index });

        uint64_t key_size = entry.key.size();
        out.write(reinterpret_cast<const char*>(&key_size), sizeof(key_size));
        out.write(entry.key.data(), key_size);
        out.write(reinterpret_cast<const char*>(&entry.offset), sizeof(entry.offset));

        current_offset_in_index += sizeof(key_size) + key_size + sizeof(entry.offset);
    }
    return summary_entries;
}

void SSTableRaw::writeSummaryToBuffer(std::ostream& out) {
    if (summary_.summary.empty()) return;

    // Format: [min_len][max_len][count][min_key][max_key][entry1]...
    uint64_t min_len = summary_.min.size();
    uint64_t max_len = summary_.max.size();
    uint64_t count = summary_.summary.size();

    out.write(reinterpret_cast<const char*>(&min_len), sizeof(min_len));
    out.write(reinterpret_cast<const char*>(&max_len), sizeof(max_len));
    out.write(reinterpret_cast<const char*>(&count), sizeof(count));
    out.write(summary_.min.data(), min_len);
    out.write(summary_.max.data(), max_len);
...
void SSTableRaw::writeBloomToBuffer(std::ostream& out) const {
    auto raw_data = bloom_.serialize();
    uint64_t len = raw_data.size();

    // Format: [velicina(uint64)][sadrzaj]
    out.write(reinterpret_cast<const char*>(&len), sizeof(len));
    out.write(reinterpret_cast<const char*>(raw_data.data()), len);
}

void SSTableRaw::writeMetaToBuffer(std::ostream& out) const {
