...
SSTableComp::SSTableComp(const std::string& dataFile,
    ...
    : SSTable(dataFile, indexFile, filterFile, summaryFile, metaFile, bmp,
        is_single_file, 
        true),          // is_compressed je uvek true za ovu klasu
    key_to_id(map),
    id_to_key(id_to_key),
    nextID(nextId)
{
...
std::vector<IndexEntry> SSTableComp::writeDataToBuffer(std::vector<Record>& sortedRecords, std::ostream& out) {
    std::vector<IndexEntry> index_entries;
    index_entries.reserve(sortedRecords.size());
    uint64_t logical_offset = 0;
    ...
    for (const auto& record : sortedRecords) {
        ...
        // Pripremi enkodirane podatke => isto kao u writeDataMetaFiles
        bool is_tombstone = static_cast<bool>(record.tombstone);
        std::string crc_varint = varenc::encodeVarint(record.crc);
        std::string ts_varint = varenc::encodeVarint(record.timestamp);
        uint32_t key_id = key_to_id.at(record.key);
        std::string key_id_varint = varenc::encodeVarint(key_id);
        const std::string& value_to_write = is_tombstone ? "" : record.value;

        // Logika serijalizacije (bez deljenja, jer pisemo u neogranicen memorijski bafer)
        Wal_record_type flag = Wal_record_type::FULL;
        uint64_t value_size = value_to_write.length();
        std::string val_size_varint = varenc::encodeVarint(value_size);

        // Serijalizuj ceo zapis direktno u stream
        out.write(reinterpret_cast<const char*>(&flag), sizeof(flag));
        out.write(reinterpret_cast<const char*>(&record.tombstone), sizeof(record.tombstone));
        out.write(crc_varint.c_str(), crc_varint.length());
        out.write(ts_varint.c_str(), ts_varint.length());
        out.write(key_id_varint.c_str(), key_id_varint.length());

        if (!is_tombstone) {
            out.write(val_size_varint.c_str(), val_size_varint.length());
            out.write(value_to_write.c_str(), value_to_write.length());
        }

        // racunamo logicki ofset
        logical_offset += sizeof(flag) + sizeof(record.tombstone) + crc_varint.length() + ts_varint.length() + key_id_varint.length();
        if (!is_tombstone) {
            logical_offset += val_size_varint.length() + value_to_write.length();
        }
    }
    return index_entries;
}

std::vector<IndexEntry> SSTableComp::writeIndexToBuffer(std::ostream& out) {
    std::vector<IndexEntry> summary_entries;
    summary_entries.reserve(index_.size());

    // Format: [broj_unosa(varint)][key_id_1(varint), offset_1(varint)]...
    std::string count_str = varenc::encodeVarint((uint64_t)index_.size());
    out.write(count_str.c_str(), count_str.length());

    uint64_t current_offset = count_str.length();

    for (const auto& entry : index_) {
        summary_entries.push_back({ entry.key, current_offset });

        uint32_t key_id = key_to_id.at(entry.key);
        std::string key_str = varenc::encodeVarint(key_id);
        std::string offset_str = varenc::encodeVarint(entry.offset);

        out.write(key_str.c_str(), key_str.length());
        out.write(offset_str.c_str(), offset_str.length());

        current_offset += key_str.length() + offset_str.length();
    }
    return summary_entries;
}

void SSTableComp::writeSummaryToBuffer(std::ostream& out) {
    // Format: [broj_unosa(varint)][min_key_id(varint)][max_key_id(varint)][key_id_1(varint), offset_1(varint)]...
    std::string count_str = varenc::encodeVarint((uint64_t)summary_.summary.size());
    out.write(count_str.c_str(), count_str.length());

    uint32_t min_key_id = key_to_id.at(summary_.min);
    uint32_t max_key_id = key_to_id.at(summary_.max);

    std::string min_key_str = varenc::encodeVarint(min_key_id);
    std::string max_key_str = varenc::encodeVarint(max_key_id);

    out.write(min_key_str.c_str(), min_key_str.length());
    out.write(max_key_str.c_str(), max_key_str.length());

    for (const auto& entry : summary_.summary) {
        uint32_t key_id = key_to_id.at(entry.key);
        std::string key_str = varenc::encodeVarint(key_id);
        std::string offset_str = varenc::encodeVarint(entry.offset);
...
void SSTableComp::writeBloomToBuffer(std::ostream& out) const {
    auto raw_data = bloom_.serialize();
    // Format: [velicina_filtera(varint)][sadrzaj_filtera]
    std::string len_str = varenc::encodeVarint((uint64_t)raw_data.size());

    out.write(len_str.c_str(), len_str.length());
    out.write(reinterpret_cast<const char*>(raw_data.data()), raw_data.size());
}


void SSTableComp::writeMetaToBuffer(std::ostream& out) const {
    // Format: [root_hash]\n[leaf1_hash]\n[leaf2_hash]...
    if (rootHash_.empty()) return;
